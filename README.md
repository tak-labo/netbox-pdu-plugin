...
A [NetBox](https://github.com/netbox-community/netbox) plugin for managing Power Distribution Units (PDUs). Links PDU devices to NetBox Device records and provides outlet/inlet monitoring and power control via vendor APIs.
...
# Run tests (Docker)
docker compose exec netbox bash -c "cd /opt/netbox/netbox-pdu && uv run pytest"
...
[NetBox](https://github.com/netbox-community/netbox) 用プラグイン。管理PDU（電源分配装置）を NetBox の Device レコードに紐付け、ベンダーAPI 経由でアウトレット・インレットの監視と電源制御を行います。
